...
[ -z "`query "select base from database_permissions where base='$opt_base_val' and user='$opt_user_val';"`" ] && error "User $opt_user_val is already disabled for database $opt_base_val" && exit 0
