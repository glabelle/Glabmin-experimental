...
( [ -n "`query "select domain from http_domains where domain='$opt_domain_val';"`" ] || [ -n "`query "select domain from https_domains where domain='$opt_domain_val';"`" ] || [ -n "`query "select domain from http_subdomains where domain='$opt_domain_val';"`" ] || [ -n "`query "select domain from https_subdomains where domain='$opt_domain_val';"`" ] ) && [ -n "$APACHE_STATUS" ] && $DAEMON_HTTP_SERVER stop
...
then
	$SCRIPTSDIR/glabmin_utils/subdomain_foreach.sh -d $opt_domain_val -n "`query "select client from domains where name='$opt_domain_val'"`" -c "< $SCRIPTSDIR/glabmin_utils/enable_subdomain.sh -d $opt_domain_val -s [SUBDOMAIN] >"
fi
...
then
    ...
    do  
    	echo "$DB_SYSTEM_POOL $opt_base_val"   
        [ -z "`mount|grep "$DB_SYSTEM_POOL$opt_base_val"`" ] && echo "action" && mount --bind $opt_dbroot_val/$opt_base_val/ $DB_SYSTEM_POOL$opt_base_val
    done
