...
then
	$SCRIPTSDIR/glabmin_utils/subdomain_foreach.sh -d $opt_domain_val -n "`query "select client from domains where name='$opt_domain_val'"`" -c "< $SCRIPTSDIR/glabmin_utils/disable_subdomain.sh -d $opt_domain_val -s [SUBDOMAIN] >"
fi
