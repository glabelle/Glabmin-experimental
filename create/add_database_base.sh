...
#inclusion des procedures communes et de la configuration.
...
[ -n "`mysql -N -u $DATABASE_ADMIN_USER -p $DATABASE_ADMIN_PASS -e "use information_schema ; SELECT SCHEMA_NAME FROM SCHEMATA where SCHEMA_NAME='$opt_base_val';"`" ] && error "Base $opt_base_val is system-dedicated"
...
mysql -N -u $DATABASE_ADMIN_USER -p $DATABASE_ADMIN_PASS -e "use mysql ; create database $opt_base_val;" && 
