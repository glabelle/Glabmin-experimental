...
[ -n "`query "select base from database_permissions where base='$opt_base_val' and user='$opt_user_val';"`" ] && error "User $opt_user_val is already allowed for database $opt_base_val" && exit 0
...
mysql -N -u $DATABASE_ADMIN_USER -p $DATABASE_ADMIN_PASS -e "use mysql ; grant ALTER, CREATE, CREATE TEMPORARY TABLES, CREATE VIEW, DELETE, DROP, INDEX, INSERT, LOCK TABLES, SELECT, UPDATE on $opt_base_val.* to '$opt_user_val'@'localhost' ;" && exit 0
