...
[ -z `echo $opt_directory_val|egrep '^[/][a-zA-Z0-9]([-._/]?[a-zA-Z0-9]+)*$'` ] && error "Invalid directory name $opt_directory_val"
[ "`echo ${#opt_directory_val}`" -gt "40"  ] && error "directory name $opt_directory_val too long"
[ ! -d "$opt_directory_val" ] && error "$opt_directory_val is not a valid directory"
[ -n "`query "select directory from http_domains_restricted where domain='$opt_domain_val' and directory='$opt_directory_val'"`" ] && error "directory $opt_directory_val already restricted"
